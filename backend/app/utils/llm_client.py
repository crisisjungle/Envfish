...
class LLMClient:
    ...
    def _is_deepseek_reasoner(self) -> bool:
        return "deepseek-reasoner" in (self.model or "").lower()
    ...
    def chat_json(
        ...
        if self._is_deepseek_reasoner():
            ...
            return fallback_client.chat_json(
                messages=messages,
                temperature=temperature,
                max_tokens=max_tokens
            )

        # DeepSeek reasoning models may return empty content when forced into
        # OpenAI's response_format=json_object mode. For those models, request
        # plain text and parse JSON locally.
